            ...
            <div class="table-responsive">
                <table class="table table-hover table-bordered table-striped" id="programTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-center">{{ ($programs->currentPage() - 1) * $programs->perPage() + $index + 1 }}</td>
                            <td class="text-center">
                                @if($program->thumbnail && file_exists(public_path('upload/camp/' . $program->thumbnail)))
                                    <img src="{{ asset('upload/camp/' . $program->thumbnail) }}" alt="Thumbnail" class="img-thumbnail" style="max-width: 80px; max-height: 60px;">
                                @else
                            ...
                            <td>
                                <div class="price-info">
                                    <small>Per Hari: Rp {{ number_format($program->harga_perhari, 0, ',', '.') }}</small><br>
                                    <small>1 Minggu: Rp {{ number_format($program->harga_satu_minggu, 0, ',', '.') }}</small><br>
                                    <small>1 Bulan: Rp {{ number_format($program->harga_satu_bulan, 0, ',', '.') }}</small>
                                    <a href="#" class="show-more-prices" data-program-id="{{ $program->id }}">Lihat lebih banyak...</a>
                                    <div class="more-prices" id="more-prices-{{ $program->id }}" style="display:none;">
                                        <small>2 Minggu: Rp {{ number_format($program->harga_dua_minggu, 0, ',', '.') }}</small><br>
                                        <small>3 Minggu: Rp {{ number_format($program->harga_tiga_minggu, 0, ',', '.') }}</small><br>
                                        <small>2 Bulan: Rp {{ number_format($program->harga_dua_bulan, 0, ',', '.') }}</small><br>
                                        <small>3 Bulan: Rp {{ number_format($program->harga_tiga_bulan, 0, ',', '.') }}</small><br>
                                        <small>6 Bulan: Rp {{ number_format($program->harga_enam_bulan, 0, ',', '.') }}</small><br>
                                        <small>1 Tahun: Rp {{ number_format($program->harga_satu_tahun, 0, ',', '.') }}</small>
                                    </div>
                                </div>
                            </td>
                            <td>
                                ...
                                <a href="#" class="show-more-facilities" data-program-id="{{ $program->id }}">Lihat selengkapnya</a>
                                @endif
                            </td>
                            <td class="text-center">
                                <div class="btn-group btn-group-sm" role="group">
                                    <a href="{{ route('admin.programs.camp.edit', $program->id) }}" class="btn btn-warning" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.programs.camp.destroy', $program->id) }}" method="POST" onsubmit="return confirmDelete(event)" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
...
<style>
    ...
    .show-more-prices, .show-more-facilities {
        font-size: 0.8rem;
        color: #007bff;
        cursor: pointer;
    }
